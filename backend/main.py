...
def recall_tool(query, limit=DEFAULT_RECALL_LIMIT):
    """Searches the hierarchical memory for relevant information."""
    results = recall_results(memory, query)
    return format_recall_results(results, limit=limit)
...
def format_actor_prompt(message, history):
    ...
    prompt += "DETERMINISTIC TOOL DISCIPLINE:\n"
    prompt += "- For arithmetic, calculations, percentages, or explicit math expressions, use calculate.\n"
    prompt += "- For calendar logic, day-of-week, days-until, UTC time, or elapsed-time questions, use date_time_reason.\n"
    prompt += "- For unit conversions, use convert_units.\n"
    prompt += "- For word counts, character counts, line counts, uppercase, lowercase, or reverse text operations, use text_utility.\n"
    prompt += "- Do not guess or estimate when a deterministic tool applies. The tool result is the source of truth.\n"
